...
using IVersion_Task_Procedure_LinkDomainService = QTD2.Domain.Interfaces.Service.Core.IVersion_Task_Procedure_LinkService;


namespace QTD2.Application.Services.Shared
{
    public class Version_Task_Procedure_LinkService : IVersion_Task_Procedure_LinkService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAuthorizationService _authorizationService;
        private readonly IStringLocalizer<Version_Task_EnablingObjective_LinkService> _localizer;
        private readonly IVersion_Task_Procedure_LinkDomainService _versionTaskProcedureLinkService;
        private readonly UserManager<AppUser> _userManager;

        public Version_Task_Procedure_LinkService(IHttpContextAccessor httpContextAccessor, IAuthorizationService authorizationService, IStringLocalizer<Version_Task_EnablingObjective_LinkService> localizer, IVersion_Task_Procedure_LinkDomainService versionTaskProcedureLinkService, UserManager<AppUser> userManager)
        {
            _httpContextAccessor = httpContextAccessor;
            _authorizationService = authorizationService;
            _localizer = localizer;
            _versionTaskProcedureLinkService = versionTaskProcedureLinkService;
            _userManager = userManager;
        }

        public async Task<Version_Task_Procedure_Link> CreateAsync(Version_Task_Procedure_LinkCreateOptions options)
        {
            var obj = (await _versionTaskProcedureLinkService.FindAsync(x => x.Version_ProcedureId == options.Version_ProcedureId && x.Version_TaskId == options.Version_TaskId)).FirstOrDefault();
            string versionNumber = "";
            if (obj == null)
            {
                versionNumber = "1.0";
                obj = new Version_Task_Procedure_Link(options.Version_TaskId, options.Version_ProcedureId, versionNumber);
            }
            else
            {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _versionTaskProcedureLinkService.AddAsync(obj);
        ...
        public async System.Threading.Tasks.Task DeleteAsync(int id)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _versionTaskProcedureLinkService.UpdateAsync(obj);
                if (!validationResult.IsValid)
                {
                    throw new System.ComponentModel.DataAnnotations.ValidationException(message: string.Join(',', validationResult.Errors));
                }
        ...
        public async Task<List<Version_Task_Procedure_Link>> GetAsync()
        {
            var obj_list = await _versionTaskProcedureLinkService.AllAsync();
        ...
        public async Task<Version_Task_Procedure_Link> GetAsync(int id)
        {
            var obj = await _versionTaskProcedureLinkService.GetAsync(id);
            if (obj != null)
            {
        ...
        public async Task<Version_Task_Procedure_Link> UpdateAsync(int id, Version_Task_Procedure_LinkUpdateOptions options)
        {
            var obj = await GetAsync(id);
            ...
            if (result.Succeeded)
            {
                obj.Version_ProcedureId = options.Version_ProcedureId;
