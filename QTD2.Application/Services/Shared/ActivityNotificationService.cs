...
namespace QTD2.Application.Services.Shared
{
    ...
    {
        private readonly IActivityNotificationDomainService _activityNotificationService;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAuthorizationService _authorizationService;
        private readonly IStringLocalizer<ActivityNotificationService> _localizer;
        private readonly UserManager<AppUser> _userManager;

        public ActivityNotificationService(IHttpContextAccessor httpContextAccessor, IAuthorizationService authorizationService, IStringLocalizer<ActivityNotificationService> localizer, IActivityNotificationDomainService activityNotificationService, UserManager<AppUser> userManager)
        {
        ...
        public async System.Threading.Tasks.Task ActiveAsync(int id)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _activityNotificationService.UpdateAsync(obj);
        ...
        public async Task<ActivityNotification> CreateAsync(ActivityNotificationsCreateOptions options)
        {
            var obj = (await _activityNotificationService.FindAsync(x => x.Title == options.Title)).FirstOrDefault();
            if (obj == null)
            {
                obj = new ActivityNotification(options.Title);
            }
            else
            {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _activityNotificationService.AddAsync(obj);
                if (!validationResult.IsValid)
                {
                    throw new System.ComponentModel.DataAnnotations.ValidationException(message: string.Join(',', validationResult.Errors));
                }
        ...
        public async Task<List<ActivityNotification>> GetAsync()
        {
            var obj_list = await _activityNotificationService.AllAsync();
        ...
        public async Task<ActivityNotification> GetAsync(int id)
        {
            var obj = await _activityNotificationService.GetAsync(id);
            if (obj != null)
            {
        ...
        public async Task<ActivityNotification> UpdateAsync(int id, ActivityNotificationsUpdateOptions options)
        {
            var obj = await GetAsync(id);
