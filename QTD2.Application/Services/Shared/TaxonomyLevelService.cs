...
{
    public class TaxonomyLevelService : ITaxonomyLevelService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAuthorizationService _authorizationService;
        private readonly IStringLocalizer<TaxonomyLevelService> _localizer;
        private readonly ITaxonomyLevelDomainService _taxonomyLevelService;
        private readonly UserManager<AppUser> _userManager;

        public TaxonomyLevelService(IHttpContextAccessor httpContextAccessor, IAuthorizationService authorizationService, IStringLocalizer<TaxonomyLevelService> localizer, ITaxonomyLevelDomainService taxonomyLevelService, UserManager<AppUser> userManager)
        {
        ...
        public async System.Threading.Tasks.Task ActiveAsync(int id)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _taxonomyLevelService.UpdateAsync(obj);
        ...
        public async Task<TaxonomyLevel> CreateAsync(TaxonomyLevelCreateOptions options)
        {
            var obj = (await _taxonomyLevelService.FindAsync(x => x.Description.ToLower().Trim() == options.Description.ToLower().Trim())).FirstOrDefault();
            if (obj == null)
            {
                obj = new TaxonomyLevel(options.Description);
            }
            else
            {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _taxonomyLevelService.AddAsync(obj);
                if (!validationResult.IsValid)
                {
                    throw new System.ComponentModel.DataAnnotations.ValidationException(message: string.Join(',', validationResult.Errors));
                }
        ...
        public async Task<List<TaxonomyLevel>> GetAsync()
        {
            var obj_list = await _taxonomyLevelService.AllAsync();
        ...
        public async Task<TaxonomyLevel> GetAsync(int id)
        {
            var obj = await _taxonomyLevelService.GetAsync(id);
            if (obj != null)
            {
        ...
        public async Task<TaxonomyLevel> UpdateAsync(int id, TaxonomyLevelUpdateOptions options)
        {
            var obj = await GetAsync(id);
