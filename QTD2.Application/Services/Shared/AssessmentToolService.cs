...
using IAssessmentToolDomainService = QTD2.Domain.Interfaces.Service.Core.IAssessmentToolService;

namespace QTD2.Application.Services.Shared
{
    public class AssessmentToolService : IAssessmentToolService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAuthorizationService _authorizationService;
        private readonly IStringLocalizer<AssessmentToolService> _localizer;
        private readonly IAssessmentToolDomainService _assessmentToolService;
        private readonly UserManager<AppUser> _userManager;

        public AssessmentToolService(IHttpContextAccessor httpContextAccessor, IAuthorizationService authorizationService, IStringLocalizer<AssessmentToolService> localizer, IAssessmentToolDomainService assessmentToolService, UserManager<AppUser> userManager)
        {
        ...
        public async System.Threading.Tasks.Task ActiveAsync(int id)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _assessmentToolService.UpdateAsync(obj);
        ...
        public async Task<AssessmentTool> CreateAsync(AssessmentToolCreateOptions options)
        {
            var obj = (await _assessmentToolService.FindAsync(x => x.Name == options.Name)).FirstOrDefault();
            if (obj == null)
            {
                obj = new AssessmentTool(options.Name);
            }
            else
            {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _assessmentToolService.AddAsync(obj);
                if (!validationResult.IsValid)
                {
                    throw new System.ComponentModel.DataAnnotations.ValidationException(message: string.Join(',', validationResult.Errors));
                }
        ...
        public async Task<List<AssessmentTool>> GetAsync()
        {
            var obj_list = await _assessmentToolService.AllAsync();
        ...
        public async Task<AssessmentTool> GetAsync(int id)
        {
            var obj = await _assessmentToolService.GetAsync(id);
            if (obj != null)
            {
        ...
        public async Task<AssessmentTool> UpdateAsync(int id, AssessmentToolUpdateOptions options)
        {
            var obj = await GetAsync(id);
