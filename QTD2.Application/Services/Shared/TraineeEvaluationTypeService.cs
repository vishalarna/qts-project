...
using IEvaluationTypeDomainService = QTD2.Domain.Interfaces.Service.Core.ITraineeEvaluationTypeService;

namespace QTD2.Application.Services.Shared
{
    public class TraineeEvaluationTypeService : ITraineeEvaluationTypeService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAuthorizationService _authorizationService;
        private readonly IStringLocalizer<TraineeEvaluationTypeService> _localizer;
        private readonly IEvaluationTypeDomainService _evaluationTypeService;
        private readonly UserManager<AppUser> _userManager;

        public TraineeEvaluationTypeService(IHttpContextAccessor httpContextAccessor, IAuthorizationService authorizationService, IStringLocalizer<TraineeEvaluationTypeService> localizer, IEvaluationTypeDomainService evaluationTypeService, UserManager<AppUser> userManager)
        {
        ...
        public async System.Threading.Tasks.Task ActiveAsync(int id)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _evaluationTypeService.UpdateAsync(obj);
        ...
        public async Task<TraineeEvaluationType> CreateAsync(TraineeEvaluationTypeCreateOptions options)
        {
            var obj = (await _evaluationTypeService.FindAsync(x => x.Name == options.Name)).FirstOrDefault();
            if (obj == null)
            {
                obj = new TraineeEvaluationType(options.Name);
            }
            else
            {
            ...
            if (result.Succeeded)
            {
                ...
                var validationResult = await _evaluationTypeService.AddAsync(obj);
                if (!validationResult.IsValid)
                {
                    throw new System.ComponentModel.DataAnnotations.ValidationException(message: string.Join(',', validationResult.Errors));
                }
        ...
        public async Task<List<TraineeEvaluationType>> GetAsync()
        {
            var obj_list = await _evaluationTypeService.AllAsync();
        ...
        public async Task<TraineeEvaluationType> GetAsync(int id)
        {
            var obj = await _evaluationTypeService.GetAsync(id);
            if (obj != null)
            {
        ...
        public async Task<TraineeEvaluationType> UpdateAsync(int id, TraineeEvaluationTypeUpdateOptions options)
        {
            var obj = await GetAsync(id);
