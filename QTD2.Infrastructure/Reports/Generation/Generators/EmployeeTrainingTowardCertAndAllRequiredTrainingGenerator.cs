...
{
    public class EmployeeTrainingTowardCertAndAllRequiredTrainingGenerator : ReportModelGenerator
    {
        ...
        public EmployeeTrainingTowardCertAndAllRequiredTrainingGenerator(
        ...
        {
            string templatePath = "EmployeeTrainingTowardCertandAllRequiredTraining.cshtml";
            ...
            var nercCertificationIds = nercCertifications.Select(n => n.Id).Distinct().ToList();
            ...
            var emergencyCalculator = _certificationFulfillmentCalculatorFactory.CreateEmergencyResponseCalculator();
            var emergencyCertIds = additionalCertifications.Where(x => x.InternalIdentifier == "Emergency Response").Select(e => e.Id).ToList();
            var emergencyFulfillmentStatuses = await emergencyCalculator.GetFulfillmentStatusesForYearAsync(employeeIds,emergencyCertIds, DateTime.Now.Year);

            //Limit Emergency Response, Reg, Reg2, and Other CertificationFulfillmentStatuses to be only one of each if multiple exist
            var firstEmergencyResponse = emergencyFulfillmentStatuses.GroupBy(bc => bc.EmployeeId).Select(g => g.OrderByDescending(c => c.IssueDate).FirstOrDefault(c => c.CertificationId == additionalCertifications.Where(c => c.InternalIdentifier == "Emergency Response").Select(c => c.Id).FirstOrDefault())).Where(c => c != null).ToList();
