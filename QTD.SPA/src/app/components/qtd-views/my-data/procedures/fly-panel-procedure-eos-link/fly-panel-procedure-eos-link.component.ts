...
@Component({
  selector: 'app-fly-panel-procedure-eos-link',
  templateUrl: './fly-panel-procedure-eos-link.component.html',
  styleUrls: ['./fly-panel-procedure-eos-link.component.scss'],
})
export class FlyPanelProcedureEosLinkComponent implements OnInit {
  showLoader=true;
        ...
        cat['enablingObjective_SubCategories'].forEach((subCat, j) => {
          ...
          subCat['enablingObjective_Topics'].forEach((topic, k) => {
            ...
            topic['enablingObjectives'].forEach((eo, l) => {
              l++;
              !eo['isMetaEO'] && !eo['isSkillQualification'] && eo.active ? treeData[i].children[j]?.children[k + this.notTopicEOs]?.children?.push({
                children: [],
...
/*
  makeEOTreeWithoutTopicDataSource(res: any) {
    ...
    for (var data in res) {
      treeDatawithoutTopic[data] = {
        id: res[data]['id'],
        description:res[data]['number'] + ' - ' + res[data]['title'],
  ...
  LinkToProcedure() {
    ...
    this.procService.LinkEOs(this.id, options).then(async (res: any) => {
      ...
      this.alert.errorToast("Error Linking " + await this.transformTitle('Enabling Objective') + " to " + await this.transformTitle('Procedure') + " " + err);
      this.showLinkEOLoader = false;
    })
  }
