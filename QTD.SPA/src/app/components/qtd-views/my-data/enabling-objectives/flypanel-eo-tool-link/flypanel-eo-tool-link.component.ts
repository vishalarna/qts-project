...
@Component({
  selector: 'app-flypanel-eo-tool-link',
  templateUrl: './flypanel-eo-tool-link.component.html',
  styleUrls: ['./flypanel-eo-tool-link.component.scss']
})
export class FlypanelEoToolLinkComponent implements OnInit {
  ...
  makeToolTreeData(res: ToolCategory[]) {
    ...
    res.forEach((toolCat: ToolCategory, index: any) => {
      ...
      toolCat.tools.forEach((tool: Tool) => {
        treeData[index]["children"].push({ description: tool.name, active: tool.active, checkbox: !this.alreadyLinked.includes(tool.id), id: tool.id });
      })
    });
  ...
  linkToSQ(){
    ...
    this.eoService.LinkTool(this.eoId,options).then(async (_)=>{
      this.alert.successToast("Linked Selected " + await this.labelPipe.transform('Tool') + "(s) To EO");
      this.closed.emit('fp-eo-tool-link-closed');
    }).finally(()=>{
      this.spinner = false;
    })
  }
