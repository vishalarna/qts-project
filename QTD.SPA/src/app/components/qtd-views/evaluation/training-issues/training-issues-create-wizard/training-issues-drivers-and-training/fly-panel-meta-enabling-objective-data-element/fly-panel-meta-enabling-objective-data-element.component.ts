...
@Component({
  selector: 'app-fly-panel-meta-enabling-objective-data-element',
  templateUrl: './fly-panel-meta-enabling-objective-data-element.component.html',
  styleUrls: ['./fly-panel-meta-enabling-objective-data-element.component.scss']
})
export class FlyPanelMetaEnablingObjectiveDataElementComponent implements OnInit {
  ...
  metaEnablingObjectiveDataSource = new MatTreeNestedDataSource<EOTree>();
  metaEoControl = new NestedTreeControl<EOTree>((node: any) => node.children);
  originalSource = new MatTreeNestedDataSource<EOTree>();
  metaEoCheckListSelection = new SelectionModel<any>(true);
  ...
  expandAndSelectNodeById(linkedId: string) {
      ...
      if (node.children) {
        for (let child of node.children) {
          if (findAndExpand(child)) {
            this.metaEoControl.expand(node);
            return true;
          }
        }
      }
    ...
    this.metaEnablingObjectiveDataSource.data.forEach((data) => {
      findAndExpand(data);
    });
  }
  ...
  readyEOTreeData(res: any[]) {
    this.notTopicEOs = 0;
    if (res.length === 0) {
      this.metaEnablingObjectiveDataSource.data = [];
    } else {
      var treeData: EOTree[] = [];
      res.forEach((cat, i) => {
        treeData.push({
      ...
      this.metaEoControl.dataNodes = Object.assign(treeData);
      this.metaEnablingObjectiveDataSource.data = Object.assign([], treeData);
      this.originalSource.data = Object.assign([], treeData);
      Object.keys(this.metaEnablingObjectiveDataSource.data).forEach((key: any) => {
        this.setParent(this.metaEnablingObjectiveDataSource.data[key], undefined);
  ...
  onMetaEOChange(selected: any, node: EOTree) {
    this.metaEoCheckListSelection.clear();
    if (selected) {
      this.metaEoCheckListSelection.select(node);
      this.linkedId = node.id;
    }
  }
  ...
  filterMetaEO() {
    ...
    this.metaEnablingObjectiveDataSource.data = filteredData;
    this.metaEoControl.dataNodes = filteredData;
    ...
    if (this.filterSearchString.length > 0) {
      this.metaEoControl.expandAll();
    } else {
      this.metaEoControl.collapseAll();
    }
