...
@Component({
  selector: 'app-fly-panel-trainingprogram-link-ila',
  templateUrl: './fly-panel-trainingprogram-link-ila.component.html',
  styleUrls: ['./fly-panel-trainingprogram-link-ila.component.scss']
})
export class FlyPanelTrainingprogramLinkILaComponent implements OnInit {
  ...
  makeIlaTreeDataSource(res: any) {
    ...
    if (res.length == 0) {
      ...
      const filteredProviders = res.map(provider => {
        if (provider.ilAs) {
          const activeIlas = provider.ilAs.filter(ila => ila.active);
  ...
  linkILA() {
    ...
    this.trainingProgramsrvc.linkILA(this.id, options).then(async (_) => {
      this.alert.successToast("Linked Selected " + await this.labelPipe.transform('ILA') + " to Training Program");
      this.refresh.emit();
      this.closed.emit('fp-task-Ila-link-closed');
    }).finally(() => {
